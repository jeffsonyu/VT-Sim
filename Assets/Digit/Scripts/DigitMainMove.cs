...
public class DigitMainMove : MonoBehaviour
{
    public ArticulationBody root;
    public ArticulationBody move;
    public CollisionEvent collider;
    public ControllerAttr hand;
    ...
    public static DigitMainMove Instance;
    Texture2D tex;
    ...
    private void Awake()
    {
        ...
        processPath = $"D:/dataset/ycb_touch_inf/{System.DateTime.Now.ToString("yyyyMMddHHmmss")}";
        //AssetManager.Instance.AddListener("GetObjImage", GetObjImage);
        ...
        AssetManager.Instance.AddListener("SetPos", SetPos);
        AssetManager.Instance.AddListener("Move", (msg) => StartCoroutine(Move(msg)));
        AssetManager.Instance.AddListener("Grab", Grab);
    ...
    void SetPos(IncomingMessage msg)
    {
        Vector3 position = new Vector3(msg.ReadFloat32(), msg.ReadFloat32(), msg.ReadFloat32());
        root.transform.position = position;
        root.transform.LookAt(Vector3.zero, Vector3.back);
        root.TeleportRoot(root.transform.position, root.transform.rotation);
        ArticulationDrive x = move.xDrive;
        x.target = 0;
        move.xDrive = x;
        hand.GripperOpen();
    }
    IEnumerator Move(IncomingMessage msg)
    {
        ...
        AssetManager.Instance.SendMessage("DigitHandMoveDone");
        yield break;
    }
    void Grab(IncomingMessage msg)
    {
        hand.GripperClose();
    }
